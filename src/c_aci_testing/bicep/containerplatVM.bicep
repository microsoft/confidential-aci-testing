      ...
      script: concat(
        ...
        join(
          [
            ...
            'echo \'# Script written by containerplatVM.bicep\' >> C:\\storage_get.ps1'
            'echo \'if ($uri -notmatch "^https://[a-zA-Z0-9]+\\.blob\\.core\\.windows\\.net/") { throw "Invalid Azure storage URL: $uri" }\' >> C:\\storage_get.ps1'
            ...
            'echo \'param([string]$uri, [string]$inFile, [switch]$binary)\' > C:\\storage_put.ps1'
            'echo \'# Script written by containerplatVM.bicep\' >> C:\\storage_put.ps1'
            'echo \'if ($uri -notmatch "^https://[a-zA-Z0-9]+\\.blob\\.core\\.windows\\.net/") { throw "Invalid Azure storage URL: $uri" }\' >> C:\\storage_put.ps1'
            ...
            'echo \'# However, we do not want to use Get-Content for binary files, as it turns non-valid Unicode chars into replacement characters depending on Powershell version.\' >> C:\\storage_put.ps1'
            'echo \'if ($binary) {\' >> C:\\storage_put.ps1'
            'echo \'  Invoke-RestMethod -Uri $uri -Method PUT -Headers $headers -InFile $inFile\' >> C:\\storage_put.ps1'
            'echo \'} else {\' >> C:\\storage_put.ps1'
            'echo \'  Invoke-RestMethod -Uri $uri -Method PUT -Headers $headers -Body (Get-Content -Raw $inFile)\' >> C:\\storage_put.ps1'
            'echo \'}\' >> C:\\storage_put.ps1'
